...
def streaming_chatbot(
    ...
    """
    Chatbot hội thoại nhiều lượt trong terminal với streaming:
...
def run_assistant(
    ...
    while True:
        ...
        stream = retry_with_backoff(make_stream)
        reply_pieces = []
        for chunk in stream:
            delta = chunk.choices[0].delta.content or ""
            print(delta, end="", flush=True)
        ...
        cost_dict = estimate_cost(user_msg, reply_text, model=OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        ...
        g = (r.get("gpt4o_response", "")[:27] + "...") if len(r.get("gpt4o_response", "")) > 30 else r.get("gpt4o_response", "")
        m = (r.get("mini_response", "")[:27] + "...") if len(r.get("mini_response", "")) > 30 else r.get("mini_response", "")
        gl = f"{r.get('gpt4o_latency', 0.0):.2f}s"
        ml = f"{r.get('mini_latency', 0.0):.2f}s"
        lines.append(f"{p:<30} | {g:<30} | {m:<30} | {gl:<15} | {ml:<15}")
    return "\n".join(lines)
